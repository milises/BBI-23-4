//2 уровень. номер 8
//Сделать абстрактный класс, и от него создать 2-х наследников:
//«Хоккей» и «Баскетбол». В баскетболе вместо
//штрафного времени, используется поле «количество фолов» (0 – 5).
//Переопределить базовый метод «Добавить штраф» в каждом
//из классов-наследников, чтобы он изменял соответствующее классу поле.
//Игроки с 4 и 5 фолами исключаются.
using System;

abstract class Task
{
    protected string[] Players { get; set; }
    protected int[] Penalties { get; set; }
